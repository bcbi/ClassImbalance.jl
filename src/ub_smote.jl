...
function smote(X, y; k::Int, under::T, pct_minority::T) where {T <: AbstractFloat}
    ...
    under = needed/(n1 * pct_minority/100) * 100

    # println("Percent undersampling: $under")
    X_new, y_new = ub_smote(X, y, pct_minority, k, under)
    return (X_new, y_new)
end
